...
- **Guess the Song** – the host plays the first seconds of a song (YouTube intro from fixed video IDs, the first song loads during the category vote, the next one while the current round plays, so YouTube ads run silently in the background; the sound only comes on while the song itself runs; iTunes preview as fallback), everyone types artist and title on their phone (no suggestions for the artist; once the artist is right, the title field suggests that artist's songs; small typos are fine; anime and movie names are strict – Dragon Ball ≠ Dragon Ball Z, Conan alone ≠ Detektiv Conan; unlimited tries for the artist, 3 for the title), a "⚑ Report song" button (host while loading, playing and at the reveal, phones at the reveal) sends broken or wrong songs to the developer (Firebase rooms/SONG/actions) and leaves that song out on this device – Guess the Song settings can bring reported songs back; fastest right answer scores most (title 12, 9, 7 …, artist 8, 6, 5 …; a wrong guess locks for 2 s). First 10 s of the intro, then 30 s to guess; if nobody has the title, the song plays again until the 2 minutes are up; after the first right title everyone gets 10 more seconds. Categories: 2000s, 2010s, Current hits, K-Pop, Disney (German), Anime (German), Anime (Japanese), Studio Ghibli – 100 songs each (Anime (Japanese) 114 incl. the late-night anime from VOX, MTV and VIVA), 102 German anime intros (several seasons / openings per series, the season is shown at the reveal), 47 Ghibli songs (guess the movie; German, English and Japanese titles count) – 763 songs.
